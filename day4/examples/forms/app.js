...
app.post('/register', function (req, res) {
  ...
  req.check('firstName', "name is empty").notEmpty();
  req.check('middleI', "Mname sholud be 1 letter").isLength({
    min: 1,
    max: 1
  });
  req.check('lastName', "Last name is empty").notEmpty();
  req.check('dOB', "date is after").isAfter();
  req.check('pwd', "pasword is empty").notEmpty();
  req.check('pwdRepeat', "paswword mismatch").equals(req.body.pwd);
