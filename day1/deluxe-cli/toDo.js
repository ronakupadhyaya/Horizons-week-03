...
program.option('-p, --priority <n>', 'Give priority to item', parseInt);
...
function showTasks(){
    ...
    var task;
    if (program.id){
      task = ("Task #" + (program.id) + ' Priority ' + data[program.id -1].priority + ": "
    ...
    data.forEach(function (item, i){
     task = ("Task #" + (i+1) + ' Priority ' + data[i].priority + ": " + data[i].name);
     console.log(task);
   })
  }
}
