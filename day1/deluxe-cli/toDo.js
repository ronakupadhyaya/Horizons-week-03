...
program.command('show')
  .description("Shows all todo list items")
  .action(showTasks);
...
program
  ...
  .option('-p,--priority <n>','Add a Priority', parseInt)
  .option('-c,--completed','Shows all completed todos');
...
function showTasks(){
  ...
  if (program.id){
    ...
    data.forEach(function(item){
      console.log("Task #"+i+" Priority "+item.priority+": "+ item.name);
      i++
    })
  }
...
function deleteTask(){
  return data.splice(program.id-1,1);
}
