...
function showTasks(){
  var showID = program.id || false;
  if (showID) {
    var task = data[showID-1]
    console.log("Task #"+ showID + " Priority " + task.priority + ": " + task.name);
  } else {
    data.forEach(function(task, id) {
      console.log("Task #"+ (id+1) + " Priority " + task.priority + ": " + task.name);
    })
  }
}
