...
function showTasks(){
  // If flag value for id, display the task only
  var argArr = process.argv;

  if(argArr[3] === '-i'){
    var testNum = argArr[4];
    var priorityNum = data[Number.parseInt(program.id) - 1].priority;
    var name = data[Number.parseInt(program.id) - 1].name;
    console.log("Task #" + program.idea + " Priority " + priorityNum + ": " + name);
  } else {
    data.forEach(function(item, index){
      var testNum = index + 1;
      var priorityNum = item.priority;
      var name = item.name;
      console.log("Task #" + testNum + " Priority " + priorityNum + ": " + name);
    })
  }
