...
function showTasks(){
  ...
  if (id === 0) {
    for (var i=0; i<data.length;i++) {
      ...
      var taskNum = i+1;
      console.log("Task #" + taskNum + " Priority " +currentTask.priority + ": "+ currentTask.name)
    }
  } else {
    for (var i=0;i<data.length;i++) {
      if (data[i].priority === data.length-id+1) {
        var needTask = data[i];
        break;
      }
    }
    console.log("Task #" + id + " Priority " + needTask.priority + ": "+ needTask.name)
  }
}
