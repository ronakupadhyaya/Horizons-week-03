...
program
  .option('-p, --priority <n>', 'Specify proirty of task', parseInt);

// Third one will be '-c' or '--completed'
program
  .option('-c, --completed <n>', 'status of task', parseInt);
...
function completeTask(){
  data[program.id-1].complete = true;
  console.log('Task #'+program.id + ' Priority '+data[program.id-1].priority  + ': ' + data[program.id-1].name + ' '+data[program.id-1].complete);
}

function toggleTask(){
  if (data[program.id-1].complete){
    data[program.id-1].complete=false;
  }
  else{
    data[program.id-1].complete=true;
  }
