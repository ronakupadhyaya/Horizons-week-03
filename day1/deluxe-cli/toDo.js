...
program
  .option('-c, --completed <n>', 'Specify task that is completed', parseInt);
...
function showTasks(){
  if(program.id){
    ...
    if(task.completed){
      console.log(`Task #${program.id} Priority ${task.priority}: ${task.name}, Completed`);
    } else {
    ...
    var id = i+1;
    if(task.completed){
      console.log(`Task #${id} Priority ${priority}: ${name}, Completed`);
    }
  } else {
