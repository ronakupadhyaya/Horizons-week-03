...
program
  ...
  .option('-c, --completed', 'List all completed tasks');
...
function showTasks(){
  ...
  if(program.completed){
    var filtered = data.filter( (task, i) => {
      if(i === 0) return true;
      return false;
    });
    var ans = ``;
    filtered.forEach( (task, i) => {
      var temp = i+1;
      ans += `Task #${temp} Priority ${task.priority}: ${task.name} Status: ${task.completed}\n`;
    });
    console.log(ans);
    return;
  }
  ...
  if(process.argv.length === 3){
    ...
    var currTask = data[index-1];
    console.log(`Task #${index} Priority ${currTask.priority}: ${currTask.name} Status: ${task.completed}`);
  }
...
function deleteTask(){
  // YOUR CODE HERE
  var index = process.argv[process.argv.length - 1];
  data.splice(index-1, 1);
}

function toggleCompleted(){
  var index = process.argv[process.argv.length - 1];
  data[index-1].completed = !data[index-1].completed;
}
