...
program.command('toggleCompleted')
  .description("Marking tasks as completed")
...
program
  .option('-p, --priority <n>', 'Specify the proority of task',parseInt);

program
  .option('-c, --completed', 'Showing that the task is completed');
...
function showTasks(){
  ...
  if (program.completed) {
    data.forEach(function(task, ind) {
      if (task.completed) {
        console.log("Task #" + (ind + 1) + " Priority " + task.priority + ": " + task.name);
      }
    })
  }
  else if (program.id) {
    var task = data[program.id - 1];
    ret = ret + "Task #" + program.id + " Priority " + task.priority + ": " + task.name;
    console.log(ret)
  } else {
    data.forEach(function(task, ind) {
      //console.log("Task #" + (ind + 1) + " Priority " + task.priority + ": " + task.name);
