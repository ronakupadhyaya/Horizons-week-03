...
function showTasks(){
  if(!isNaN(program.id)) {
    if(program.id >= data.length) throw 'error id value not listed'
    console.log('Task #%d Priority %d: %s',(program.id+1), data[program.id].priority, data[program.id].name );
  }
