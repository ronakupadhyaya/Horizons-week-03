...
function displayContacts() {
  ...
  console.log(columnify(data, {
    dataTransform: function(num) {
      return (num === '-1' ? '-None-' : num);
    },
    config: {
      name: {
        headingTransform: function(heading) {
          return (`contact_${heading}`).toUpperCase();
        }
      },
      number: {
        headingTransform: function(heading) {
          return (`phone_${heading}`).toUpperCase();
        }
      }
    }
  })); //UNCOMMENT
...
function addContact() {
  ...
  if (isName(newName) && isNum(num)) {
    var names = data.map(function(contact) {
      return contact.name;
    })
    if (names.includes(newName)) {
      console.log(`${name} already in Contact Book`);
    } else {
      num === undefined ? num = -1 : num = parseInt(num);
      data.push({
        name: newName,
        number: num
      });
...
function updateContact() {
  // YOUR CODE HERE
  var og = process.argv[1];
  var newval = process.argv[2];
  var names = data.map(function(contact) {
    return contact.name;
  })
  if (!names.includes(og)) {
    ...
    var i = names.indexOf(og);
    if (isName(newval)) {
      //change name here
      data[i].name = newval;
    } else if (isNum(newval)) {
      //change number here
      data[i].number = parseInt(newval);
    } else {
      console.log('Invalid contact format');
    }
  }
