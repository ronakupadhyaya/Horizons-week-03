...
function fileReader(csvFilePath){
  // YOUR CODE HERE
  var csv_data = fs.readFileSync(csvFilePath).toString();
  //console.info(csv_data);

  var array = csvjson.toObject(csv_data);
  //console.info(array);
...
function parser(arr){
  ...
  for(var i = 0; i < investments.length; i++){
    investments[i].originalInvestment = parseInt(investments[i].originalInvestment);
    investments[i].valueToday = parseInt(investments[i].valueToday);
  }
  return investments;
