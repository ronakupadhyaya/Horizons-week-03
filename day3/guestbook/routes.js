...
// data.save(data): Write the given data to disk.
...
router.get('/posts', function(req, res) {
    ...
    posts.sort(function(a, b){
        var y = new Date(a.date)
        var z = new Date(b.date)
        return y - z;
    });
...
function validate(req) {
    req.checkBody('title', 'Insert title').notEmpty();
    req.checkBody('body', 'Insert body').notEmpty();
}

router.post('/posts', function(req, res) {
    validate(req); //validates the request and returns validation errors
    var errors = req.validationErrors();
    if (errors) {
        res.render('posts', { error: errors })
    }
    if (req.cookies.username) {
        var renderArray = data.read()
        req.body.date = new Date();
        renderArray.push(req.body)
        console.log(renderArray)

        data.save(renderArray)
