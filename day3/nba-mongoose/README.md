...
1. Follow the steps above to get ready to use mlab for the `nba-mongoose` project if you haven't already.
...
1. From the command line fun `node playerAdd.js`. This is a small program we provided to populate your database with some info to start out with.
...
We want to do everything we need to in this step such that in the next one we'll be able to make an endpoint on our server that will retrieve all the players in the mongo database and respond with them.
...
At this point there's not much to show for your efforts, but you should be able to at least run your code with `npm start` without getting an error. If you uncommented the lines logging the mongoose connection and you included the `console.log` in the callback of `app.listen` you should see some output indicating that those two pieces worked.

### Part 3: GET /  --> Player info

Now we will write an express route which, when hit, will pull info from the Player collection and respond to the requester with that info.
  ...
  <details>
    <summary>Express Hint</summary>
     ...
     app.get('/', function() {
       ...
       // res.send after getting the data
       // When it comes to async callbacks,
...
1. Following the example laid out in `model/player.js`, add a new model **Roster** into the model folder that has the following attributes:
