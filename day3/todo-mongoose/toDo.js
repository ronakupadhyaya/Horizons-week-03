...
var ToDoItemItem = mongoose.model('todoitemitem', {
  name: String,
  priority: String,
  completed: Boolean
});

var ToDoItem = new ToDoItemItem({
  name: "Go to bed",
  priority: "Urgent",
  completed: false
});
...
function showTasks() {
  ...
  if (program.task) {
    ToDoItemItem.find({
      ...
      console.log('todoitemitems', todoitems);
    });
  } else {
    ToDoItemItem.find(function(error, todoitems) {
      if (error) {
        console.log("Can't find ToDoItems", error);
      } else {
        console.log('todoitemitems', todoitems);
      }
    });
