...
// ToDoItem.find(function(error, ToDoItems) {
//   if (error) {
//     console.log("Can't find tasks", ToDoItems);
//   } else {
//     console.log('Tasks', ToDoItems);
//   }
// });
...
function addTask(){
  ...
  task.save(function(err){
    if(err){
      console.log('There is an error, homie!');
    }
    else{
...
function showTasks() {
  ...
  if(process.argv.length <= 3){
    ...
    console.log('This is the task name: ' + name);
    console.log("Printing one task!");
    ToDoItem.find({name: name}, function(err, ToDoItems){
      if(err){
        console.log('There is an error, homie :(', ToDoItems);
      }
      else{
        console.log('You good :)', ToDoItems);
      }
    })
  }
  mongoose.connection.close();
...
function deleteTask(){
  ...
  var name = parseArgs();
  if(name){
    ToDoItem.remove({name: name}, function(err, ToDoItems){
      if(err){
        console.log('There is an error, homie :(', ToDoItems);
      }
      else{
