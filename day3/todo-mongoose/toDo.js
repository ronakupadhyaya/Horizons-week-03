...
program
  .option('-t, --task <t>', 't is the name of the test')
...
program.parse(process.argv); // removes flags from string
...
function addTask() {
  ...
  task.save(function(err, saved) {
    if (err) {
      console.log("Was not saved", error);
    } else {
...
function deleteTask() {
  ...
  ToDoItem.remove(program.task, function(err) {
    if (err) {
      console.log("could not remove your shit");
    }
  })
}
}
