...
program.command('add')
.description("Create Tasks")
.action(addTask); // add = create task
...
function addTask(){
  ...
  var name = parseArgs(); // get the name of the task

  // make new instances of mongoose.model. create a variable called task, set name, priority and completed
  // set those properties equal
  // boolean called true?
  ...
  //var ToDoItem = new mongoose.Schema({ name: 'string', completed: true; });
  // YOUR CODE HERE
...
function showTasks() {
  ...
  // if(program.task){
  //   ToDoItem.find(name:program.task, function(error,items){
  //     for(i=0; i<items.length; i++){
  //       console.log("Task: %d, Priority %d; Completed:%s",items[i].name, items[i].priority, items[i].completed);
  //     }
  //      mongoose.connection.close();
  //   })
  //   else{
...
function deleteTask(){
  ...
  // YOUR CODE HERE
  if(!program.task){// if not given program.task (!)
    console.log("No task given");
  }
  else{
    ToDoItem.remove({name:program.task}, function(err, items){
      console.log(program.task+ " has been removed bitch");
    });
  }
  mongoose.connection.close();
}
