...
function showTasks() {
  ...
  if (name) {
    ToDoItem.find({ name: name }, function (err, elem) {
      if (err) {
        console.log('Find Error', err);
      }
      else {
        elem.forEach(function (a) {
          console.log("Task:", a.name, "Priority:", a.priority, "Completed:", a.completed);
        });
      }
    })
  } else {
    ToDoItem.find(function (err, elem) {
      if (err) {
        console.log('Find Error', err);
      }
      else {
...
function deleteTask() {
  ...
  if (name) {
    ToDoItem.remove({ name: name }, function (err) {
      if (err) {
        console.log('Find Error', err);
      }
      else {
