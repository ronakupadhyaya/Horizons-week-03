...
var ToDoList = db.model('TodoList', {
  name: String,
  priority: String,
  completed: Boolean
});
  ...
  if (program.task) {
    ToDoList.find({
      ...
      if (err) {
        ...
        task.forEach(function (taksInList) {
          console.log(`Task: ${taksInList.name}, Priority: ${taksInList.priority}, Completed: ${taksInList.completed}`)
        });
      }
    });
  } else {
