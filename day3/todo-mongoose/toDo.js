...
var ToDoItem = mongoose.model('toDo', schema)
...
if (program.task) {
  ToDoItem.findOne({name: program.task}, function(err, task) {
    if(err) {
  ...
  ToDoItem.find(function(err, task) {
    if(err) {
      ...
      task.forEach(function(current) {
        console.log(`Task: ${current.name}, Priority: ${current.priority}, Completed: ${current.completed}`)
      })
      mongoose.connection.close();
...
function deleteTask(){
  ...
  if(program.task) {
      ...
      if(err) {
        console.log("This task does not exist", err);
      }
