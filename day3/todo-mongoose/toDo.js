...
program
  .option('-t, --task <p>', 'Specify task');
program
  .option('-c, --completed', 'Specifies if task is completed');
...
function addTask(){
  ...
  var completed = program.completed || false;
  ...
  task.save(function(err, task) {
    if (err) {
      console.log("Can't add task", err);
...
function showTasks() {
  ...
  TodoItem.find({name: program.task}, function(err, tasks) {
    if (err) {
      console.log("Can't find task", err);
      mongoose.connection.close();
    } else {
      tasks.forEach(function (task) {
        if (task.name === program.task) {
          console.log("Task: "+ program.task + ", Priority: " + task.priority + ", Completed: " + task.completed);
        }
      })
...
function deleteTask(){
  ...
  if (program.task) {
    TodoItem.remove({name: program.task}, function(err, tasks) {
      if (err) {
        console.log("Can't find task to remove", err)
        mongoose.connection.close();
      } else {
        // console.log(tasks);
        console.log("Task: "+ program.task);
        mongoose.connection.close();
      }
    })
  } else {
