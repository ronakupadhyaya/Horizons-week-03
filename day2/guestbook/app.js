...
app.get('/posts', function (req, res) {
  ...
  if (req.query.author) {
    var showAuthor = req.query.author;
    sortedData = sortedData.filter(function(item) {
      if (item.author) {var lower = item.author.toLowerCase()}
      return lower === showAuthor;
    })
  }
  res.render('posts', {
...
app.get('/posts/new', function(req, res) {
  ...
  if(!req.cookies.username) {
    res.status(404).send("Error 404: You're not logged in!");
  }
...
app.post('/posts', function(req, res) {
  if(!req.body.title || !req.body.body ||!req.body.date) {
    res.status(400).send("Error 404: You're missing a field!");
    return;
  }
