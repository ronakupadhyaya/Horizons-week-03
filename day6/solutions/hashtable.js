...
HashTable.prototype.hashCode = function (str){
  return hashCode(str) % this.capacity;
}
...
HashTable.prototype.put = function(key, value) {
  // YOUR CODE HERE
  var hash = this.hashCode(key);
  if(this.pairs[hash] === undefined){
    this.pairs[hash] = [[key, value]];
    this.length++;
    return false;
  }else{
    var inserted = false;
    for(var i = 0; i < this.pairs[hash].length; i++){
      if(this.pairs[hash][i][0] === key){
        //Key already exists, updating it's value
        this.pairs[hash][i][1] = value;
        inserted = true;
        return true;
      }
    }
    //Key didn't exist. Appending to array.
    if(inserted === false){
      this.pairs[hash].push([key,value]);
      this.length++;
    }
  }
  return false;
...
HashTable.prototype.delete  = function(key){
  var hash = this.hashCode(key);
  if(this.pairs[hash]){
    if(this.pairs[hash].length === 1 && this.pairs[hash][0][0] === key){
...
HashTable.prototype.containsValue = function(value) {
  // YOUR CODE HERE
  for (var hash in this.pairs) {
    for (var key in this.pairs[hash]) {
      if (this.pairs[hash][key][1] === value) {
        return true;
      }
    }
  }
...
HashTable.prototype.keysValues = function(fn) {
  for (var hash in this.pairs) {
    for (var key in this.pairs[hash]) {
      fn([this.pairs[hash][key][0], this.pairs[hash][key][1]])
    }
  }
}
